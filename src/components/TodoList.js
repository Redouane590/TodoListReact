...
function TodoList({ todoList, deleteTodo, toggleTodo, toggleTodoEdit, editTodo, selectTodo }) {
  return todoList.length ? (
    <ul>
      ...
      <TodoItem key={todo.id} todo={todo} deleteTodo={deleteTodo} toggleTodo={() => toggleTodo(todo.id)} toggleTodoEdit={() => toggleTodoEdit(todo.id)} selectTodo={() => selectTodo(todo.id)}/> )}
    </ul>
